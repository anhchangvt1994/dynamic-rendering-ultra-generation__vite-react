"use strict"; function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; } function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }var _path = require('path');
...
const _deleteUserDataDir = async (path) => {
  if (path) {
    try {
      ...
      ), 'optionalAccess', _3 => _3.exec, 'call', _4 => _4('deleteResource', [path])])
    } catch (err) {
      console.error(err)
    }
  }
...
const loadCapacityTest = async (url) => {
  ...
  if (browser.isConnected()) {
    ...
    console.log('-------------')
    if (_optionalChain([response, 'optionalAccess', _5 => _5.status, 'call', _6 => _6()]) === 200) {
