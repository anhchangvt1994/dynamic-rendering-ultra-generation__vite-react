...
var _utils3 = require('./CacheManager.worker/utils'); var _utils4 = _interopRequireDefault(_utils3);
 ...
 const isAvailablePointsTo = async (url) => {
  const urlInfo = new URL(url)
  const routeInfo =
    _nullishCoalesce(_nullishCoalesce(_optionalChain([_serverconfig2.default, 'access', _8 => _8.routes, 'access', _9 => _9.list, 'optionalAccess', _10 => _10[urlInfo.pathname]]), () => (
    _optionalChain([_serverconfig2.default, 'access', _11 => _11.routes, 'access', _12 => _12.custom, 'optionalCall', _13 => _13(url)]))), () => (
    (_serverconfig2.default.routes )))
 ...
 const getOtherUrlsBaseOnDevice = (url) => {
  const urlInfo = new URL(url)
  const routeInfo =
    _nullishCoalesce(_nullishCoalesce(_optionalChain([_serverconfig2.default, 'access', _14 => _14.routes, 'access', _15 => _15.list, 'optionalAccess', _16 => _16[urlInfo.pathname]]), () => (
    _optionalChain([_serverconfig2.default, 'access', _17 => _17.routes, 'access', _18 => _18.custom, 'optionalCall', _19 => _19(url)]))), () => (
    (_serverconfig2.default.routes )))

  if (!routeInfo) return []

  const routePreviewInfo = routeInfo.pointsTo || routeInfo.preview
  const content = _nullishCoalesce(_optionalChain([routePreviewInfo, 'optionalAccess', _20 => _20.content]), () => ( routeInfo.content))
