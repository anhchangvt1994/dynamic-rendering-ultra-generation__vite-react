...
const startServer = async () => {
  if (isMainThread) {
    ...
    Promise.resolve().then(() => _interopRequireWildcard(require('./utils/CleanerService'))).catch((err) => {
    ...
    const createWorkerSafe = (order, port, retryCount = 0) => {
      try {
        const worker = new Worker(__filename, {
          workerData: { order, port },
        })
        _createWorkerListener(worker)
        _ConsoleHandler2.default.log(`Worker ${order} started on port ${port}`)
        return worker
      } catch (err) {
        _ConsoleHandler2.default.error(`Failed to create worker ${order} (attempt ${retryCount + 1}):`, err)
        if (retryCount < 5) {
          // Retry sau 5 giây khi EAGAIN (hết tài nguyên thread)
          setTimeout(() => createWorkerSafe(order, port, retryCount + 1), 5000)
        } else {
          _ConsoleHandler2.default.error(`Worker ${order} failed after 5 retries — giving up`)
        }
        return null
      }
    }
    ...
    const port = await _PortHandler.findFreePort.call(void 0, _nullishCoalesce(_optionalChain([workerData, 'optionalAccess', _3 => _3.port]), () => ( 4040)))
    ...
    app.listen(Number(port), (token) => {
      if (token) {
        console.log(`Server started port ${port}. Press Ctrl+C to quit`)
        _optionalChain([process, 'access', _4 => _4.send, 'optionalCall', _5 => _5('ready')])
        process.title = 'web-scraping'
      } else {
        console.log(`Failed to listen to port ${port}`)
      }
