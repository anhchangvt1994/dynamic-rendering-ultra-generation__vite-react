...
;(async () => {
  ...
  geoip = await Promise.resolve().then(() => _interopRequireWildcard(require('geoip-lite')))
})()
 ...
 function detectLocale(req) {
  if (
    ['true', 'TRUE', '1'].includes(
      process.env.DISABLE_DETECT_LOCALE 
    ) ||
    !geoip ||
    !req
  )
  ...
  if (acceptLanguage) {
    ...
    ;[clientLang, clientCountry] = (() => {
      ...
      // Safety check to ensure favoriteAcceptLanguage is defined
      if (!favoriteAcceptLanguage) {
        return [LOCALE_INFO_DEFAULT.lang, LOCALE_INFO_DEFAULT.country]
      }
  ...
  if (_serverconfig2.default.locale.enable) {
    const cookies = _CookieHandleruws.getCookieFromRequest.call(void 0, req)
    const path = url
