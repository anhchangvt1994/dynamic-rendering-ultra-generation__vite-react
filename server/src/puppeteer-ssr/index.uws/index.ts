    ...
    _app.get('/*', async function (res, req) {
      ...
      if (
        ...
        const html = fs.readFileSync(
          path.resolve(__dirname, '../../../403-forbidden.html'),
          'utf8'
        )
        ...
        if (botInfo.isBot) {
          ...
          try {
            ...
            if (!res.writableEnded) {
              const html = fs.readFileSync(
                path.resolve(__dirname, '../../../500-server-error.html'),
                'utf8'
              )
        ...
        if (req.getHeader('accept') === 'application/json') {
          ...
          try {
            ...
            if (result?.status === 200) {
              ...
              html =
                fs.readFileSync(
                  result?.response ??
                    path.resolve(__dirname, '../../../503-maintain.html'),
                  'utf8'
                ) || ''
            }
          } catch (err) {
          ...
          try {
            ...
            if (!res.writableEnded) {
              const html =
                fs.readFileSync(
                  path.resolve(__dirname, '../../../404-not-found.html'),
                  'utf8'
                ) || ''
              res.cork(() => {
                res
